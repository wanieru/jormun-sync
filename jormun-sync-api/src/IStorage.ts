...
export interface IStorage
{
    ...
    getAuthorizedValues(userId: number, app: string, keys: string[]): Promise<{ [key: string]: Data }>;
    getAuthorizedKeys(userId: number, app: string): Promise<{ [key: string]: { timestamp: number, public: Publicity, sharedWith: number[] } }>;
    getUserKeys(userId: number, app: string): Promise<{ [key: string]: number }>;
    getAllUserValues(userId: number): Promise<{ [key: string]: Data }>;
    ...
    setPublic(userId: number, app: string, fragments: { [fragment: string]: Publicity }): Promise<void>;
    ...
    getAllInvitationKeys(app: string, userId: number): Promise<{ [guestTokenId: string]: string[] }>;
    getInvitationKeys(app: string, hash: string): Promise<{ userId: number, fragment: string }[]>;
};